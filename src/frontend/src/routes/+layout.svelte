<script lang="ts">
	...
	async function stopSync() {
		...
		await api_fetch("anki/stop_morphemes_sync", {
			method: "PUT"
		});
	}
...
{#if sync_task_context.sync_task}
	<CenteredPopup>
		<div class="flex flex-col items-center">
			...
			{#if !["FAILURE", "SUCCESS", "CANCELLED"].includes(sync_task_context.sync_task.status)}
		...
		<button
			...
			onclick={stopSync}>
			{["FAILURE", "SUCCESS", "CANCELLED"].includes(sync_task_context.sync_task.status)? "Close": "Stop" }
		</button>
	</CenteredPopup>
{/if}
